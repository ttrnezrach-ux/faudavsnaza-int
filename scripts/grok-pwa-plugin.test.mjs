...
test("share query localizes og title description and image", () => {
  const site = {
    title: "עברית",
    ...
    image: "/og.jpg",
    locales: {
      en: { title: "Fauda 5 vs NAZA", description: "Same method, two works." },
    },
  };
  const en = localizeOgSite(site, "?lang=en&work=compare&tab=map");
  assert.equal(en.title, "Fauda 5 vs NAZA");
  assert.equal(en.description, "Same method, two works.");
  assert.equal(en.image, "/og-en.jpg");
  const naza = localizeOgSite(site, "lang=he&work=naza");
  assert.equal(naza.image, "/works/naza.jpg");
  const html = injectGrokPwaHead("<html><head></head></html>", {
    host: "faudaint.grok.me",
    site,
    search: "?lang=en&work=compare",
  });
  assert.match(html, /property="og:title" content="Fauda 5 vs NAZA"/);
  assert.match(html, /og-en\.jpg/);
  assert.match(html, /lang=en.+work=compare/);
});
