...
**Persistence.** Set `DATABASE_URL` to a Postgres connection string (Neon works). Migrations in `migrations/` apply on deploy and again the first time the app queries that database, so attaching Neon creates the tables without another manual step. Without that variable, `/office` says new counts are not persistent instead of presenting them as a lasting total.

**What counts.** Visitor totals, charts, and the anomaly line include only browsers that run the page script. Crawlers and headless clients (Googlebot, bingbot, Facebook, WhatsApp, Slack, Twitter, curl, python-requests, uptime checks, Vercel screenshot/preview, and an empty user agent), plus prefetch and prerender requests, are stored in `bot_hits` and shown as a separate office counter. A browser that unlocks `/office` gets a 400-day `fauda_owner` cookie and is left out of those totals from then on; the office switch “Exclude this device” sets or clears that cookie for a browser that will not log in again. Requests with `Authorization: Bearer <OFFICE_MONITOR_TOKEN>` or `X-Internal-Check: 1` never count, and that header marks the browser for a day so later script calls from the same check stay out too.

**Monitoring.** `GET /api/office/traffic` returns JSON: `total`, `baseline`, `counted`, `lastHour`, `last24h`, `days` (14 UTC dates), `anomaly` (`today`, `average`, `stddev`, `zScore`, `deviationPct`, `spike`), `durable`. `anomaly` compares today with the mean and standard deviation of the previous 13 days. Send `Authorization: Bearer <token>`. The token is the env var `OFFICE_MONITOR_TOKEN`. If that variable is unset the route responds 503. A missing or wrong token responds 401. The body has no IP addresses or other personal data.

**Office IP log.** `/office` shows the visitor’s full IP next to the page they opened. That column lives only in the locked office. It is not rendered on the public site and is not included in the monitoring JSON. Addresses are kept for 90 days (`IP_RETENTION_DAYS` in `src/lib/visit-counter.ts`), then cleared; older rows keep a masked hint. Clicks (tabs, sections, outbound links, share, language, contact, accessibility) and time on site are written to the same store as the visit counter: Postgres when `DATABASE_URL` is set, otherwise the ephemeral fallback, with the same “not persistent” banner.
